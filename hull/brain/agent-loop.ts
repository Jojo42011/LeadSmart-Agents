...
- Fraud: VOIP lines, Google Voice/TextNow-style virtual carriers, IPQS fraud score ≥85, the same caller ID under 3+ publishers, cross-vertical recycling, and coordinated attacks are the core signals. Only CONNECTED calls (duration > 0) feed fraud detection. Cross-vertical is Seth's rule: same caller inside ONE service vertical = normal shopping; across 3+ different service categories in 7 days = recycled fake lead. Coordinated attack is the STRONGEST signal: same caller under 3+ publishers within 5 minutes (any duration). A caller spraying MORE than 8 publishers in 7 days is a robocaller, auto-reclassified to the no-connect watchlist, never a fraud flag. Geo clusters: 10+ scanned calls from one city in 24h (e.g. Broken Arrow, OK / 539) are surfaced separately. Publisher risk = 60% flag rate + 40% worst severity. Nothing is ever blocked automatically.
